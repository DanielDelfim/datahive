...
st.caption(
    "Os scripts abaixo **escrevem** os JSONs PP. As abas apenas **leem** e exibem."
)
...
with col1:
    if st.button("🔁 Carregar scripts (Resumo)", use_container_width=True):
        ...
        cmds = [
            ...
            [
                sys.executable,
                str(base / "carregar_preco_compra.py"),
                "--regiao",
                regiao,
            ],
            ...
            [
                sys.executable,
                str(base / "agregar_precos_min_max.py"),
                "--regiao",
                regiao,
            ],
        ]
        ...
        st.success(
            "Scripts executados. Role a página para ver logs e as abas atualizadas."
        )
...
with col2:
    st.selectbox(
        ...
        key="prec_regiao",
        index=(
            ["sp", "mg"].index(st.session_state.get("prec_regiao", "sp"))
            if st.session_state.get("prec_regiao") in ["sp", "mg"]
            else 0
        ),
    )
