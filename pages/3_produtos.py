...
# --- Ações (dispara SCRIPT autor do JSON) ---
st.divider()
st.subheader("Atualização do PP")
colA, colB = st.columns([1, 2])
with colA:
    if st.button("🔁 Atualizar PP a partir do Excel", use_container_width=True):
        # Garante UTF-8 no stdout/stderr do subprocesso (evita 'charmap' no Windows)
        env = os.environ.copy()
        ...
        if res.returncode == 0:
            ...
            if res.stdout:
                with st.expander("Ver log"):
                    st.code(res.stdout)
            # Recarrega os dados na mesma execução
            st.cache_data.clear()
            st.rerun()
        else:
