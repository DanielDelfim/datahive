# app/utils/vendas/aggregator.py
...
def _orders_paid(rows: Iterable[Dict[str, Any]]) -> float:
    """Soma 'paid_amount' deduplicando por order_id (pega o maior visto)."""
    seen = {}
    for r in rows:
        oid = r.get("order_id")
        if oid is None: 
...
def window_sums(rows: List[Dict[str, Any]], days: int,
    ...
    if mode == "ml":
        ...
        # fallback para possíveis outros modos no futuro
        win_from, win_to = ml_window_bounds(days)
        sub = rows_in_ml_window(rows, days, date_field=date_field)
...
def all_windows(rows: List[Dict[str, Any]],
...
def per_mlb(rows: List[Dict[str, Any]],
            windows: Iterable[int] = (7, 15, 30),
            date_field: str = "date_approved",
            *, mode: str = "ml") -> Dict[str, Any]:
    ...
    for mlb, lst in groups.items():
        w = {}
        for d in windows:
            w[str(d)] = window_sums(lst, d, date_field=date_field, mode=mode)
